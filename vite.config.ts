...
/**
 * MOBILE_BUILD=1 produces the bundle Capacitor wraps: a client-only SPA with no
 * Worker and no server rendering, because a packaged app has no server to call.
 * The web build is untouched — the landing page keeps its server rendering, and
 * /admin keeps its server functions.
 */
const mobile = env("MOBILE_BUILD") === "1";
...
export default defineConfig({
  ...
  // Turning nitro off for mobile is what makes the output a plain static folder
  // rather than a Cloudflare Worker bundle.
  nitro: mobile ? false : webNitro,
});
